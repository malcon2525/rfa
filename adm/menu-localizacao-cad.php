...
if (!empty($alterar)) {
	$read = read('menu_localizacao', "WHERE id=$alterar");
	if ($read){
...
if (!empty($descricao) && !empty($enviar)) {
	...
	if (empty($id)) { // CADASTRO
		...
		$gravarBD = create ('menu_localizacao', $datas);
	} else {  //ALTERAÇÃO
		$up = array(
			"descricao" => "$descricao"
		);
		$gravarBD = update('menu_localizacao', $up, "id=$id");
	}
}
  ...
  <div class="card ">
    ...
    <div id="collapseOne" class="collapse show" role="tabpanel" aria-labelledby="headingOne" data-parent="#accordion">
      <div class="card-body">
        ...
        <form action="index.php?pag=menu-localizacao-cad" method="post">
        	...
        	<div class="d-flex justify-content-end " >
	        	<button type="button" name="voltar" value="voltar" class="btn btn-secondary mr-2" onclick="window.location = 'index.php?pag=menu-localizacao-lista'">Voltar</button>
		...
		<?php } else {?>
	 	...
	 	<div class="d-flex justify-content-end">
	       	<button type="button" name="listar" value="listar" class="btn btn-dark mr-2" onclick="window.location = 'index.php?pag=menu-localizacao-lista'">Listar</button>
	       	<button type="button" name="voltar" value="voltar" class="btn btn-dark mr-2" onclick="window.location = 'index.php?pag=menu-localizacao-cad'">Efetuar Outro Cadastro</button>
		</div>
		<?php }?>
