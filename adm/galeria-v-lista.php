  ...
  <div class="card ">
    ...
    <div id="collapse<?=$a?>One" class="collapse show" role="tabpanel" aria-labelledby="heading<?=$a?>One" data-parent="#accordion">
      ...
      <div class="card-body">
        ...
        <div class="row"> 
          <div class="col-md-6">
            <a href="index.php?pag=galeria-v-cad&tipo=video" class="btn btn-primary">Inserir Galeria de Vídeo</a><br><br>
          </div>
          <div class="col-md-6">
        ...
        <table class="table table-sm table-hover">
         <tr>
           ...
           <th colspan="3"></th>
            ...
            <?
              ...
              $galeriaDB = read('galeria', "WHERE tipo like 'video'  $sqlFiltro LIMIT $inicio,$maximo");
              if ($galeriaDB) {
                foreach ($galeriaDB as $galeria) {
                  $linkEditar = "index.php?pag=galeria-v-cad&alterar=$galeria[id]&tipo=video";
                  $linkExcluir = "index.php?pag=galeria-v-lista&excluir=$galeria[id]";
                  $linkVideoCad = "index.php?pag=galeria-video-cad&galeria_id=$galeria[id]";

                  $result = mysql_query("SELECT * FROM galeria_video WHERE galeria_id = $galeria[id]");
                  $numVideo= mysql_num_rows($result);
         ...
         <tr>
           ...
           <td><a href="<?=$linkVideoCad?>" class="btn btn-dark " data-toggle="tooltip" data-placement="top" title="Inserir  Videos"><img src="images/inserirV.png">  <span class="badge badge-light"><?=$numVideo?></span></a></td>
         </tr>
        ...
        <?
          //paginacao
            ...
            $sqlFiltro = ( !empty($filtro) ? "AND titulo like '%$filtro%'" : '');
            $sqlTotal = "SELECT * FROM galeria  WHERE tipo like 'video' $sqlFiltro ";
        ...
        <nav aria-label="Page navigation example">
          <ul class="pagination justify-content-center">
            ...
            <? for ($i=1; $i<=$totalPagina; $i++) {
                $linkp="index.php?pag=galeria-v-lista&pg=$i";
                $pgSel = ( $pg == $i ? "pgSel": "" );
            ?>
